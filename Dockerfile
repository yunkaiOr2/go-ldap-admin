...
RUN release_url=$(curl -s https://api.github.com/repos/yunkaiOr2/go-ldap-admin-ui/releases/latest | grep "browser_download_url" | grep -v 'dist.zip.md5' | cut -d '"' -f 4); wget $release_url && unzip dist.zip && rm dist.zip && mv dist public/static
...
LABEL maintainer qguduemc@163.com
